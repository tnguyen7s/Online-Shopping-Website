<?php
	...
	if (!hasLogined())
	{
		header("Location: ..\login.php?action=orders\orders.php");
	}
?>
...
<html lang="en">
	<head>
		...
		<meta name="description" content="This is a page to display the customer's orders">
	...
	<body style="margin:0px; font-family: Arial,sans-serif;">
		...
		<?php
			if (isset($_GET['requestcancellation']))
			{
				echo '<div style="width: 70%; margin: 30px 15%; padding: 10px; border: solid 2px #FFAF38; border-radius: 5px;">';
			
				echo '<h2 style="color: #FFAF38;">Your request of cancellation has been sent to the seller.</h1>';
				echo '<p>Please wait for seller\'s response.</p>';
				echo '<p>Checkout the Cancelled page for further information.</p>';
				echo '</div>';
			}
		?>
		...
		<nav style="margin: 20px 15%; padding: 0px; border-bottom: solid 1px #DDD;">
			<ul style="display: flex; list-style-type: none; justify-content: flex-start; margin: 0px;"> 
				<li style="padding:5px 30px; border-bottom: solid 2px #B12704;"><a style="text-decoration-line:none; color:black; font-weight: 600;" href="">Orders</a></li>
				<li style="padding:5px 30px;"><a style="text-decoration-line:none; color: #007185;" href="notYetShippedOrders.php">Not yet shipped</a></li>
		...
		<main style="margin: 0px 15%; display: flex; flex-direction: column; font-size: 0.8em">
			...
			$orders = getPlacedOrdersFromCustomer($customerId);
				...
				if ($orderId!=null){
					...
					echo '<div class="moreAboutOrder" style="display: flex; width: 100%; justify-content: space-around; margin: 5px;">';
						...
						foreach ($items as $itemId => $itemInfo) {
							...
							echo '<form method=POST action="..\products\productreview.php">';
							echo '<input type="text" style="display:none" name="productId" value="'.$itemId.'">';
							echo '<button type="submit" style="color: black; background-color:#FFD814; border-color: #FCD200; border-radius: 19px; padding: 10px; text-decoration-line: none; margin-right: 20px;cursor: pointer; border: none;">Write a product review</button>';
							echo '</form>';

							echo '<form method=POST action="returnitem.php">';
							echo '<input type="text" style="display:none" name="orderId" value="'.$orderId.'">';
							echo '<button type="submit" style="color: black; background-color:#FFD814; border-color: #FCD200; border-radius: 19px; padding: 10px; text-decoration-line: none; margin-right: 20px;cursor: pointer; border: none;">Return item</button>';
							echo '</form>';
						...
						if ($orderStatusId==OrderStatusEnum::WaitingForSellerResponse)
						{
							$orderInfo["orderStatus"] = "Wait for the seller's responses";
						}
						else if ($orderStatusId==OrderStatusEnum::TransactionProcessing)
						{
							$orderInfo["orderStatus"] = "Transaction is being processed.";
						}
						else if (OrderStatusEnum::TransactionCompleted)
						{
							$orderInfo["orderStatus"] = "Transaction has been completed.";
						}
						...
						if ($orderStatusId==OrderStatusEnum::WaitingForSellerResponse || $orderStatusId==OrderStatusEnum::TransactionProcessing)
						{
							echo '<form method=POST action="requestCancellation.php">';
							echo '<input type="text" style="display:none" name="orderId" value="'.$orderId.'">';
							echo '<button type="submit" style="color: black; background-color: transparent; padding: 5px 50px; border: solid 1px #D5D9D9; border-radius: 8px; box-shadow: 0 2px 5px 0 rgba(213,217,217,.5); display: inline-block;text-align: center; margin-bottom: 10px; cursor: pointer">Request Cancellation</button>';
							echo '</form>';
						}
